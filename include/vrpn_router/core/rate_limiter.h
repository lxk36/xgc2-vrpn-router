...
public:
  ...
  bool shouldPublish(double now_s) const { return interval_s_ <= 0.0 || now_s >= next_publish_s_; }

  void markPublished(double now_s) { next_publish_s_ = interval_s_ > 0.0 ? now_s + interval_s_ : now_s; }
