...
public:
  PoseRoute(ros::NodeHandle& nh, RouteConfig config, int queue_size)
      : config_(std::move(config)), processor_(toProcessorConfig(config_)) {
  ...
  diagnostic_msgs::DiagnosticStatus diagnosticStatus(const ros::Time& now) const {
    const core::HealthSnapshot snapshot = processor_.snapshot(now.toSec());
...
private:
  static core::RouteProcessorConfig toProcessorConfig(const RouteConfig& config) {
    core::RouteProcessorConfig processor;
    processor.tracker_to_body = config.tracker_to_body;
    processor.field_offset = config.field_offset;
    processor.max_output_rate_hz = config.max_output_rate_hz;
    processor.health.min_input_rate_hz = config.min_input_rate_hz;
    processor.health.max_input_rate_hz = config.max_input_rate_hz;
    processor.health.stale_timeout_s = config.stale_timeout_s;
    processor.health.stuck_timeout_s = config.stuck_timeout_s;
    processor.health.stuck_position_epsilon_m = config.stuck_position_epsilon_m;
    processor.health.stuck_angle_epsilon_deg = config.stuck_angle_epsilon_deg;
    processor.health.max_translation_jump_m = config.max_translation_jump_m;
    processor.health.max_rotation_jump_deg = config.max_rotation_jump_deg;
    processor.health.max_reference_delta_m = config.max_reference_delta_m;
    return processor;
  }
  ...
  void referenceCallback(const geometry_msgs::PoseStamped::ConstPtr& msg) {
    processor_.onReference(toCorePose(msg->pose));
  }

  void poseCallback(const geometry_msgs::PoseStamped::ConstPtr& msg) {
    const ros::Time now = ros::Time::now();
    const core::RouteProcessResult result = processor_.onInput(toCorePose(msg->pose), now.toSec());
    ...
    out.pose = toRosPose(result.output);

    publisher_.publish(out);
  }

  RouteConfig config_;
  core::RouteProcessor processor_;
